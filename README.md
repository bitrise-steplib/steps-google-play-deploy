...
| `mapping_file` | The `mapping.txt` file provides a translation between the original and obfuscated class, method, and field names.  Empty by default, because an app bundle built with minification already contains its own mapping file: Google Play reads it from the bundle, so uploading one is redundant. Set this input — to `$BITRISE_MAPPING_PATH`, for example — when you deploy APKs, or when your bundle carries no mapping file.  In case of deploying [multiple artifacts](https://developer.android.com/google/play/publishing/multiple-apks.html), you can specify multiple mapping.txt files as a newline (`\n`) or pipe (`\|`) separated list. The order of mapping files should match the list of APK or AAB files in the `app_path` input. |  |  |
| `native_symbols_file` | Path to the native debug symbols file (.zip) to upload for crash symbolication. The native symbols archive should contain native libraries with debug symbols (.so files) for the app. Uploading a native symbols file is optional but recommended for native crash analysis. In case of deploying [multiple artifacts](https://developer.android.com/google/play/publishing/multiple-apks.html), you can specify multiple .zip files as a newline (`\n`) or pipe (`\|`) separated list. The order of native symbols files should match the list of APK or AAB files in the `app_path` input. |  |  |
