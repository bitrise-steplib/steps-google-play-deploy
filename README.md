...
| `mapping_file` | The `mapping.txt` file provides a translation between the original and obfuscated class, method, and field names.  Uploading a mapping file is not required when deploying an AAB as the app bundle contains the mapping file itself.  In case of deploying [multiple artifacts](https://developer.android.com/google/play/publishing/multiple-apks.html), you can specify multiple mapping.txt files as a newline (`\n`) or pipe (`\|`) separated list. The order of mapping files should match the list of APK or AAB files in the `app_path` input.  **Note**: when the **Artifact map file path** input points at an artifact map (exported by recent Android build steps), the map's per-variant pairing takes precedence over this input for every artifact the map knows — positional order is fragile with multiple variants. |  | `$BITRISE_MAPPING_PATH` |
| `artifact_map_path` | Path of the variant-keyed artifact map exported by the Android build steps (Gradle Runner, Android Build) as `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`. The JSON document records which APK/AAB files and which R8/ProGuard mapping file belong to the same build variant, so the step can upload the correct mapping file for each app artifact — Google Play accepts exactly one mapping file per version code.  When the map names a mapping file for an artifact, that pairing takes precedence over the `mapping_file` input (the step logs each decision). The `mapping_file` input is used whenever the map has nothing to say: no map, an artifact the map does not know, or a variant without a mapping file. Artifacts renamed by the Android Sign step (`*-bitrise-signed.*`) are matched back to their unsigned origin.  The map only ever improves pairing: when the input is empty, the file does not exist (for example the build ran on an older step version), or the file cannot be parsed, the step warns and behaves as before, using `mapping_file` only. |  | `$BITRISE_ANDROID_ARTIFACT_MAP_PATH` |
