...
| `track` | The track to which you want to assign the uploaded app.  Can be one of the built-in tracks (internal, alpha, beta, production), or a custom track name you added in Google Play Developer Console. |  | `alpha` |
...
| `retry_times` | Number of times to recreate the edit and retry the upload if the API fails because the edit has expired (`FAILED_PRECONDITION`: `This edit has expired, please create a new Edit.`).  Set to `0` to disable retrying. Accepts values between 0 and 10. |  | `0` |
