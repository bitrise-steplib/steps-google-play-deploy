...
| `track` | The track to which you want to assign the uploaded app.  Can be one of the built-in tracks (internal, alpha, beta, production), or a custom track name you added in Google Play Developer Console.  This input is optional: if left empty, the app is uploaded to Google Play without being assigned to any track. |  | `alpha` |
...
| `mapping_file` | The `mapping.txt` file provides a translation between the original and obfuscated class, method, and field names.  Uploading a mapping file is not required when deploying an AAB as the app bundle contains the mapping file itself.  You can provide multiple mapping files as a newline (`\n`) or pipe (`\|`) separated list, which is what `$BITRISE_MAPPING_PATH_LIST` contains when the build produced more than one.  **The order does not matter.** The Step pairs each uploaded artifact with its own mapping file by content: R8 writes a `pg_map_id` into every artifact it minifies and the same id into the header of the mapping file for that run, so the Step reads both and matches them. This works even though the Deploy directory has flattened the paths and renamed colliding `mapping.txt` files, and it is unaffected by re-signing.  A mapping file that matches none of the uploaded artifacts is not uploaded. An artifact that is not minified needs no mapping file, and one is not looked for.  R8 always writes a `pg_map_id`, in both full and compatibility mode, so any build minified by R8 is paired by content. Mapping files from an obfuscator that is not R8 — Guardsquare's standalone ProGuard plugin, DexGuard — carry no id and cannot be matched that way. If none of the provided files has one, the Step falls back to matching them to `app_path` by position, as earlier versions did.  The default lists both `$BITRISE_MAPPING_PATH_LIST` and `$BITRISE_MAPPING_PATH`, pipe-separated, so the Step works whether or not the build Step that produced the mapping files is new enough to export the list. Naming the same file twice is harmless: candidates are matched by content, and duplicates collapse. |  | `$BITRISE_MAPPING_PATH_LIST\|$BITRISE_MAPPING_PATH` |
